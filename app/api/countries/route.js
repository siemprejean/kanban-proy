// app/api/countries/countries.js
export async function GET(req){
    try {
        ...
        const res = await fetch('http://10.2.1.174:35789/general/countries', {
            headers: {
                Authorization: authHeader, // Pasamos el token recibido desde el cliente
            }
        });
        if (!res.ok) {
            return Response.json({ error: "Error al obtener paises" }, { status: res.status });
        }
