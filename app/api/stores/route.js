...
export async function GET(req){
    try {
        ...
        const res = await fetch('http://10.2.1.174:35789/general/stores', {
            headers: {
                Authorization: authHeader, // Pasamos el token recibido desde el cliente
            }
        });
        if (!res.ok) {
            return Response.json({ error: "Error al obtener tiendas" }, { status: res.status });
        }
